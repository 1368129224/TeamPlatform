function blockDrag(b) {
    ...
    var a = this;
    jQuery(this.set.dragNode).wrap("<div id='" + this.wrapId + "'></div>");
    jQuery(this.wrapNode).css({
    ...
    jQuery(this.set.boxNode).css({
        width: this.col * this.set.blockW + "px",
        height: this.row * this.set.blockH + "px"
    });
    jQuery(this.wrapNode).mousedown(function(e) {
        var c = parseInt(jQuery(this).css("left"));
        var d = parseInt(jQuery(this).css("top"));
        var f = e.pageX;
        var g = e.pageY;
        e.stopPropagation();
        a.moveFn = function(h) {
            var i = h.pageX;
            var j = h.pageY;
            jQuery(a.wrapNode).css({
                left: (c + i - f) + "px",
                top: (d + j - g) + "px"
            })
        };
        jQuery(document).on("mousemove", a.moveFn);
        if (jQuery(a.wrapNode).get(0).setCapture) {
            jQuery(a.wrapNode).get(0).setCapture()
        }
        jQuery(document).mouseup(function(j) {
            var l = parseInt(jQuery(a.wrapNode).css("left"));
            var m = parseInt(jQuery(a.wrapNode).css("top"));
            ...
            jQuery(document).off("mousemove").off("mouseup");
            jQuery(a.wrapNode).off("mouseup")
        })
    });
    if (jQuery(a.wrapNode).get(0).releaseCapture) {
        jQuery(a.wrapNode).get(0).releaseCapture()
    }
    return false
};